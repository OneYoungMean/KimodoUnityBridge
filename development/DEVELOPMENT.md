...
- `animation_analyze` accepts one or two explicit Session clips and returns numeric analysis plus unified graph-space picture evidence. `picture.tiles` selects standard tile types and `picture.output` chooses a composite PNG, independent tile PNGs, or both. Mesh-only targets do not provide Humanoid contact semantics.
