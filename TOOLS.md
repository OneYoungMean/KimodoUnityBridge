...
4. 使用一个或两个显式的 `{character,clip,role?}` 项调用 `animation_analyze`。`level` 默认是 `middle`；紧凑的 ghost/trajectory 对使用 `low`，需要额外关键帧和脚接触子图时使用 `high`。同一个不可变 Session Clip 使用相同有效 level 时，命令直接返回既有分析和图片而不重复计算。
