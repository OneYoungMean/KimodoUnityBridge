...
namespace KimodoUnityMotionTools.ProjectEditor
{
    public static class KimodoConstraintExportUtility
    {
            ...
            out string error)
        {
            ...
            try
            {
                ...
                if (!TrySamplePoseFromClipAsset(
                        ...
                        skeletonRoot,
                        marker.time,
                        KimodoConstraintMarkerEditorUtility.TimeToKimodoFrameIndex(clipRange, marker.time),
        ...
        private static KimodoConstraintJson BuildAutoConstraint(KimodoConstraintMarkerBase marker, int frameIndex, KimodoMarkerSampleResult sample)
        {
        ...
        private static KimodoConstraintJson BuildFullBodyConstraint(int frameIndex, KimodoMarkerSampleResult sample)
        {
            var full = new KimodoConstraintJson
            {
                ...
                root_positions = new List<float[]> { new[] { sample.rootPosition.x, sample.rootPosition.y, sample.rootPosition.z } },
                local_joints_rot = new List<float[][]> { ToAxisAngleArray(sample.localAxisAngles) }
            };
            return full;
        }

        private static KimodoConstraintJson BuildEndEffectorConstraint(KimodoEndEffectorConstraintMarker eeMarker, int frameIndex, KimodoMarkerSampleResult sample)
        {
            ...
            out string error)
        {
            ...
            sample = KimodoSpaceConversionUtility.ToKimodoSample(unitySample);
            if (sample == null || sample.localAxisAngles == null || sample.localAxisAngles.Count == 0)
            {
                error = $"Failed to convert sampled pose to Kimodo space at marker {globalTime:F3}s.";
                return false;
            }
