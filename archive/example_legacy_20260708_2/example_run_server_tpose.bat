...
set /a WAIT_SEC+=1
if !WAIT_SEC! geq 15 (
  echo [WARN] run_server wrapper still alive after quit grace; bridge cleanup may still be in progress.
  call :archive_file "!PID_FILE!"
  exit /b 0
)
goto wait_loop
...
set "HOST="
set "PORT="
if exist "!BRIDGE_SERVER_LOG!" (
  set "ENDPOINT="
  for /f "usebackq tokens=5" %%L in (`findstr /C:"quickserver_cli listening on" "!BRIDGE_SERVER_LOG!"`) do (
  ...
  if not defined HOST (
    set "READY_HOST="
    set "READY_PORT="
    for /f "usebackq tokens=3,4" %%A in (`findstr /C:"ready host=" "!BRIDGE_SERVER_LOG!"`) do (
      if not defined READY_HOST set "READY_HOST=%%A"
      if not defined READY_PORT set "READY_PORT=%%B"
    )
    if defined READY_HOST (
      set "HOST=!READY_HOST:host==!"
    )
    if defined READY_PORT (
      set "PORT=!READY_PORT:port==!"
    )
  )
)
