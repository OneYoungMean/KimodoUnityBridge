...
- This repository provides Unity humanoid-animation tooling through `command_dispatcher`.
...
- `DEVELOPMENT.md` is a developer-facing capability snapshot and CLI coverage matrix. Do not edit it for ordinary implementation, refactoring, documentation, or test work. Update it only when the user requests it or when the task changes public command coverage represented by the matrix.
...
- When a QuickServer change also changes the public capability summary, update its current one-line summary in both language sections of `TOOLS.md`. Update the current version line in `DEVELOPMENT.md` only under the documentation rule above.
