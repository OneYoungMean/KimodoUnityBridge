...
| 动画分析与图片证据 | `animation_analyze` | 完整 | 底层同步调用 `KimodoPlayableClipGenerationExecutionService.Analysis(...)`；接受一或两个显式角色 Clip，返回按 saliency 降序的关键帧、按持续帧数升序的 `foot_contacts`，以及 low/middle/high 的一张组合 PNG 和自描述子图。渲染不修改 Clip；不返回质量、loop seam 或 trajectory 指标。 |
