...
- `session_get_or_create` 会建立专用可见 Session 根 GameObject；角色复制到该对象下时保留源角色组件（包括已有的 `CharacterController`），仅清空 Session 副本的 `Animator.runtimeAnimatorController`，后续制作、采样、分析和渲染均限定在该对象下。根对象不保存到场景；切换或默认关闭时禁用，`session_close(keepObject=false)` 销毁 Session 根对象及其 Director。
